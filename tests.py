...
def pair_test():
    ...
    print('Number of clusters: {}'.format(kl.elbow))
    print('Number of lenses: {}'.format(len(xl)))
    ...
    plt.legend()
    plt.title('Pair Test: {} Lenses Detected'.format(len(xl)))
    plt.xlim([-size,size])
    plt.ylim([-size,size])
...
if __name__ == "__main__":
    ...
    for pair in pairs:
        ...
        #Remove maxima on the edge
        indices = []
        for i in range(len(x)):
            if xmax[i] == size or ymax[i] == size or xmax[i] == -size or ymax[i] == -size:
                indices.append(i)
        xmax = np.delete(xmax, indices)
        ymax = np.delete(ymax, indices)
    ...
    kmeans = cluster.KMeans(n_clusters=kl.elbow, random_state=42).fit(data)
    #Unpack cluster centers
    cluster_centers = kmeans.cluster_centers_

    #Create the larger map
    large_weights = np.ones((size,size,size))
    cluster_size = 20

    for i in cluster_centers:
        cluster_sources = []
        for j in range(len(xs)):
            if np.sqrt((xs[j] - i[1])**2 + (ys[j] - i[0])**2) < cluster_size:
                cluster_sources.append(j)
        print(len(cluster_sources))

        #Take the weights from each source (restricted to the cluster)
        #Only take the slice of the weights that corresponds to the cluster
        y1 = (i[0] - cluster_size)
        y2 = (i[0] + cluster_size)
        x1 = (i[1] - cluster_size)
        x2 = (i[1] + cluster_size)
        ...
        #Map the coordinates to indices in the weight map
        y1 = int(y1 + size/2)
        y2 = int(y2 + size/2)
        x1 = int(x1 + size/2)
        x2 = int(x2 + size/2)
        ...
        cluster_weights = np.ones((size,cluster_size*2,cluster_size*2))
        for j in range(len(cluster_sources)):
            try:
                chosen_weight = weights[cluster_sources[j]]
                cluster_weights*=chosen_weight[:,y1:y2,x1:x2]
            except ValueError:
                print('Value Error')

        #Embed this into the larger map
        large_weights[:,y1:y2,x1:x2] = cluster_weights

    #Remove remaining ones
    large_weights[large_weights == 1] = 0

    #Process the flexion weights
    flexion_map = process_weights(large_weights, np.linspace(1,60,size), size)


    #Find maxima within the weightmaps, output their coordinates and scores
    x,y,z = score_map(flexion_map, threshold=0.1)
    xmax, ymax = line[x], -line[y]

    #Plot the results

    plt.figure()
    plt.imshow(np.log10(flexion_map+10**-20), cmap='RdYlBu_r', extent=[-size,size,-size,size])
    plt.scatter(xl, yl, marker='*', s=200, c='g', label='Lens')
    plt.scatter(xmax,ymax, marker='o', s=z*100, c='k', alpha = 0.5, label='Maxima')
    plt.scatter(kmeans.cluster_centers_[:,0], kmeans.cluster_centers_[:,1], marker='x', s=100, c='r', label='Cluster Centers')
    plt.legend()
    plt.title('{} Lenses, {} Clusters'.format(len(xl), kl.elbow))
    plt.xlabel('x')
    plt.ylabel('y')
    plt.show()