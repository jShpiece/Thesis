...
def fit_lensing_field(sources, xmax, flags=False, use_flags=None, lens_type='SIS', z_lens=0.5, z_source=1):
    """
    ...
    Parameters:
        ...
        z_source (float, optional): Redshift of the source. Default is 1.
    ...
    lenses = pipeline.generate_initial_guess(sources, lens_type, z_lens, z_source)
    reduced_chi2 = pipeline.update_chi2_values(sources, lenses, use_flags, lens_type, z_source)
    log_step("Initial Guesses:", lenses, reduced_chi2)

    # Step 2: Optimize lens positions
    lenses = pipeline.optimize_lens_positions(sources, lenses, xmax, use_flags, lens_type, z_source)
    reduced_chi2 = pipeline.update_chi2_values(sources, lenses, use_flags, lens_type, z_source)
    ...
    lenses, _ = pipeline.forward_lens_selection(sources, lenses, use_flags, lens_type, z_source)
    reduced_chi2 = pipeline.update_chi2_values(sources, lenses, use_flags, lens_type, z_source)
    ...
    lenses = pipeline.merge_close_lenses(lenses, merger_threshold, lens_type, z_source)
    reduced_chi2 = pipeline.update_chi2_values(sources, lenses, use_flags, lens_type, z_source)
    log_step("After Merging Lenses:", lenses, reduced_chi2)

    # Step 6: Final optimization
    lenses = pipeline.optimize_lens_strength(sources, lenses, use_flags, lens_type, z_source)
    reduced_chi2 = pipeline.update_chi2_values(sources, lenses, [True, True, True], lens_type, z_source)
